...
public class ReviewFacade {
    ...
    public PaginatedData<ReviewDto> readProductReviews(int productId, Pageable pageable) {
        ...
        // DTO 매핑 후 반환
        return PaginatedData.of(reviewPage.map(review -> review.toDto(product)));
    }
    ...
    public PaginatedData<ReviewDto> readReviewHistory(
        ...
        // DTO 매핑 후 반환
        return PaginatedData.of(reviewPage.map(review -> review.toDto(productMap.get(review.getProductId()))));
    }
    ...
    public ReviewDto updateReview(long reviewId, ReviewPatchDto reviewPatchDto, List<MultipartFile> images) {
        ...
        String imageUrls =
                imageStorageService.update(review.getImageUrlArray(), reviewPatchDto.getDeleteImage(), images);
